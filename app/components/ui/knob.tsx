...
export function Knob({ value, min, max, step, onChange, label, color }: KnobProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ref={knobRef}
        className={`w-20 h-20 rounded-full bg-neutral-800 cursor-pointer relative shadow-lg border-4 ${color} overflow-hidden`}
        onMouseDown={handleMouseDown}
        onTouchStart={handleTouchStart}
      >
        <motion.div
          className="w-2 h-10 bg-white absolute left-1/2 bottom-1/2 origin-bottom rounded-full shadow-md"
