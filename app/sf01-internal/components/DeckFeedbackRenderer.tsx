...
function buildSlots(uniforms: UniformSpec[]) {
...
export function DeckFeedbackRenderer({
  effect,
  values,
  resetKey = 0,
  audioRef,
  ...
  const computeMat = useMemo(() => {
    const slots = buildSlots(effect.uniforms)
  ...
  useFrame((state) => {
    ...
    if (!seededRef.current) {
      const dst = pingRef.current === 0 ? fboA : fboB
      if (seedMat) {
